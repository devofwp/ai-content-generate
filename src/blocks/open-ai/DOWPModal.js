...
const DOWPModal = ({isOpen, setOpen}) => {
	...
	const handleInsert = () => {
		if ('gutenberg' === dowpParams.editor_type) {
			...
			window.parent.send_to_editor(aiResponse.content);
												...
												<>
													{
														aiResponse.content ?
															...
															<>
																...
																<div
																	className={`response-text ${aiResponse.status === 'error' && 'has-error'}`}>
																	<div className='text'>
																		{aiResponse.status === 'ok' &&
